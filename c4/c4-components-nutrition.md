# C4 Level 3 — Component Diagram: Nutrition & Health Management (Domain 7)
...
```mermaid
C4Component
  title Component Diagram — Domain 7: Nutrition & Health Management

  Container(spa, "Single-Page Application", "HTML5/ES6/CSS", "Provides Parent Allergy Declaration UI (/parent/allergies) and Coordinator Allergy Alerts on Roster")
  ContainerDb(db, "Relational Database", "PostgreSQL 15", "Persists student_allergies, allergy_types, dish_ingredients, and dietary_alerts")
  System_Ext(sis, "School Information System", "External SIS", "Source of official student medical checkup and dietary restriction records")

  Container_Boundary(api, "Backend API Service — Domain 7") {
    Component(nutriCtrl, "Nutrition & Allergy Controller", "Express.js Router", "Exposes REST endpoints for allergy profile declaration, ingredient tagging, and conflict evaluations")
    Component(allergyProfileService, "Allergy Profile Service", "Domain Service", "Manages student medical allergy records, severity levels (Mild, Moderate, Severe), and parent declarations")
    Component(ingredientScanner, "Ingredient Restriction Scanner", "Domain Service", "Inspects recipe ingredient compositions against common high-risk allergen categories")
    Component(conflictAlertEngine, "Menu-Restriction Conflict Alert Engine", "Safety Domain Service", "Cross-references daily scheduled menu dishes against confirmed student diners to generate visual warning badges")
    Component(nutriRepo, "Nutrition Repository", "TypeORM / Data Access", "Reads and writes student health profiles, dietary restrictions, and alert logs")
  }

  Rel(spa, nutriCtrl, "Declares allergies, reviews conflict alerts on class rosters", "JSON / HTTPS")
  Rel(nutriCtrl, allergyProfileService, "Manages allergy profiles")
  Rel(nutriCtrl, ingredientScanner, "Flags restricted ingredients in dishes")
  Rel(nutriCtrl, conflictAlertEngine, "Evaluates conflicts for active meal schedule")
  Rel(nutriCtrl, nutriRepo, "Persists allergy entities and alerts")

  Rel(allergyProfileService, sis, "Syncs official medical records and allergy notes", "HTTPS / REST")
  Rel(nutriRepo, db, "Reads/writes student_allergies, dishes, ingredients, dietary_alerts", "SQL")
```
