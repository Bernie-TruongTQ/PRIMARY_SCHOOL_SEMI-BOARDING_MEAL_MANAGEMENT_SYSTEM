# C4 Level 3 — Component Diagram: Master Data & System Configuration (Domain 8)
...
```mermaid
C4Component
  title Component Diagram — Domain 8: Master Data & System Configuration

  Container(spa, "Single-Page Application", "HTML5/ES6/CSS", "Provides Administrator Academic Structure Console (/admin/academic) and Calendar Management UI (/admin/calendar)")
  ContainerDb(db, "Relational Database", "PostgreSQL 15", "Persists school_years, semesters, grades, classes, students, meal_calendars, and holidays")
  System_Ext(sis, "School Information System", "External SIS", "Master record of school years, grade levels, and student enrollments")

  Container_Boundary(api, "Backend API Service — Domain 8") {
    Component(masterCtrl, "Master Data & Config Controller", "Express.js Router", "Exposes REST endpoints for academic structure setup, classroom rosters, student profiles, and holiday calendars")
    Component(academicStructureService, "Academic Structure Service", "Domain Service", "Manages school year lifecycles, grade levels, class assignments, and student directory records")
    Component(calendarConfigService, "Meal Calendar Configuration Service", "Domain Service", "Configures lunch serving day schedules, institutional non-serving rules, and holiday calendars")
    Component(sisSyncService, "SIS Synchronization Service", "Integration Service", "Periodically reconciles student enrollment rosters and classroom movements against the external SIS master")
    Component(masterRepo, "Master Data Repository", "TypeORM / Data Access", "Executes relational lookups and constraints across academic entities")
  }

  Rel(spa, masterCtrl, "Configures academic years, classes, students, and holiday dates", "JSON / HTTPS")
  Rel(masterCtrl, academicStructureService, "Manages academic structures")
  Rel(masterCtrl, calendarConfigService, "Configures serving schedules and holidays")
  Rel(masterCtrl, sisSyncService, "Triggers sync with external SIS")
  Rel(masterCtrl, masterRepo, "Persists academic data")

  Rel(sisSyncService, sis, "Fetches school year, class, and student records", "HTTPS / REST")
  Rel(masterRepo, db, "Reads/writes school_years, grades, classes, students, meal_calendars, holidays", "SQL")
```
